...
def get_dict_by_conf(target, txt):
    ret = {}
    # conf_json, error_list = _read_json(cof_file)
    conf_json = conf_dict[target]
...
def deal_bankid_None(txt, target):
    list = get_file_list(txt)
    for i in list:
...
def get_file_list(temp = None):
    ...
    if isinstance(temp, str):
        ...
        #logger = logger_cof("onekeylog.log")
        write_log("{} no find".format(type(temp)))
...
def reset_dic(dict_a):
    for key in dict_a.keys():
        if isinstance(dict_a[key], dict):           # isinstance()  return true if the type of parameter one is the same as parameter two, otherwise return false
