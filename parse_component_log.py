...
def get_component_info(log_filename, rawdata_filename, log_cputype_re, raw_cputype_re, args_cpu):
    ...
    if os.path.exists(log_filename):
        ...
        write_log("Warning: component log file %s not found." % log_filename)
        error_list.append("Warning: component log file %s not found." % log_filename)
    return component_info, error_list

def get_fwinfo_bylog(log_filename):
    news = ""
    txt = get_file_retxt(log_filename)
    fw_info = get_dict_by_conf("fw_info", txt)  # fw_info ： log文件中的version部分数据
    if len(fw_info) == 0:
...
def get_cpudict_byall(log_filename, rawdata_filename, log_cputype_re, raw_cputype_re, cputype=None):
    ...
    sockets = []
    cpu_dict = {}
    cpu_dict = get_cpudict_verid_sockets_bylog(log_filename, log_cputype_re)
    if len(cpu_dict['cpu_info']) == 0:
        ...
        cpu_dict, news = get_cpudict_byregrawdata_argscpu(rawdata_filename, raw_cputype_re, cputype)
        if news:
            error_list.append((os.path.basename(log_filename), news))
    return cpu_dict, error_list

def get_cpudict_verid_sockets_bylog(log_filename, log_cputype_re):
    cpu_type = None
    ver_id = None
    cpu_info = []
    sockets = []
...
def get_cpudict_byregrawdata_argscpu(rawdata_filename, raw_cputype_re, cputype=None):
    cpu_type = None
    ver_id = None
    cpu_info = []
    sockets = []
