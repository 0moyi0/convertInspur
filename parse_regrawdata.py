...
from util import _read_json, merge_dic, get_inspur_reg_map, write_log, generate_file
    ...
    try:
        if datas:
            for log in datas:
                ...
                # log ： log is one data of RegRawData.json.
                ...
                for socket in sockets:  # sockets ： List of CPU quantities
                ...
                generate_file(rawdata_filename, new_crashdump, i)
                i = i + 1
...
# according to the input cha_dict、key and val(Traversing cha_data get key and val),
...
def update_cha_dict(cha_dict, key, val):
    ...
    if names:
        ...
        # print("WARNING: update_cha_dict(): unknown register %s" % key)
        write_log("WARNING: update_cha_dict(): unknown register %s" % key)
        # raise Exception("WARNING: Can't parse %s in function update_cha_dict()" % key)
    return cha_dict
...
# according to the input mca_dict、key and val(Traversing mca_data get key and val),
...
def update_mca_dict(mca_dict, key, val):
    ...
    if 'thread' not in key:
        ...
        if names:
            ...
            # print("WARNING: update_mca_dict(): unknown register %s" % key)
            write_log("WARNING: update_mca_dict(): unknown register %s" % key)
            # raise Exception("WARNING: Can't parse %s in function update_mca_dict()" % key)
    else:
...
# according to the input csr_dict、key_str、val and cpu_type(Traversing csr_data get key and val),
# return the updated mca_dict.
def update_csr_dict(csr_dict, key, val, cpu_type):
    val = val.replace("Errcode:", "CC:")
    reg_map = get_inspur_reg_map(cpu_type)
    name = re.match('(cpu[0-9]{1,2})_(.*)', key, re.M | re.I)
    if name:
        ...
        for reg in reg_map:
            if (len(csrlist) >= len(reg[5].split("_"))) and ("_".join(csrlist[(len(csrlist) - len(reg[5].split("_"))):]) == reg[5].strip()):
                temp = re.match('rootport([0-9])', key.lower(), re.M | re.I)
                if temp:
                    num = 1 + temp.group(1)
                    bus = "0" + num
                else:
        ...
        # print("WARNING: update_csr_dict(): unknown register %s" % key)
        write_log("WARNING: update_csr_dict(): unknown register %s" % key)
        # raise Exception("WARNING: Can't parse %s in function update_csr_dict()" % key)
    return csr_dict
